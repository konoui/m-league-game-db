...
reach_classification AS (
    SELECT
        ...
        -- 赤ドラなし判定
        CASE WHEN ps.hand LIKE '%0m%' OR ps.hand LIKE '%0p%' OR ps.hand LIKE '%0s%' THEN 1 ELSE 0 END AS has_red,
        -- ドラなし判定（リーチ前に公開された表ドラと手牌を照合）
        CASE WHEN EXISTS (
            SELECT 1 FROM dora_indicator_event die
            JOIN event e_dora ON die.event_id = e_dora.id
            WHERE e_dora.kyoku_id = rd.kyoku_id
              AND e_dora.event_order < rd.reach_order
              AND INSTR(',' || ps.hand || ',', ',' || die.dora || ',') > 0
        ) THEN 1 ELSE 0 END AS has_dora,
        ...
        -- リーチ以外の確定役あり判定（七対子・タンヤオ等が付く待ちがあるか）
        CASE WHEN EXISTS (
            ...
            WHERE tam.tenpai_event_id = rd.discard_event_id
              AND yn.name NOT IN ('立直', '一発', '裏ドラ', '門前清自摸和', '嶺上開花', 'ドラ', '赤ドラ')
...
stats AS (
    SELECT
        ...
        SUM(CASE WHEN rc.is_gukei = 1 AND rc.has_red = 0 AND rc.has_dora = 0 AND rc.has_kan = 0 AND rc.has_other_yaku = 0
