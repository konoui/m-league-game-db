...
streak_groups AS (
    SELECT 
        ...
        date,
        kyoku_order,
        -- 連続性を判定するためのグループ番号を生成
        kyoku_order - ROW_NUMBER() OVER (PARTITION BY actor_player_id, game_id ORDER BY kyoku_order) AS streak_group
    FROM player_agari
),
-- 連続あがり回数を集計する
consecutive_agari AS (
    SELECT 
        actor_player_id,
        game_id,
        player_name,
        ...
        stage,
        date,
        COUNT(*) AS consecutive_agari_count,
        MIN(kyoku_order) AS streak_start_kyoku,
        MAX(kyoku_order) AS streak_end_kyoku
    FROM streak_groups
    GROUP BY actor_player_id, game_id, player_name, team_name, season_year, stage, date, streak_group
    HAVING COUNT(*) >= 2
)
-- 最終結果: 連続あがり回数上位5件を出力
SELECT 
    ...
    team_name AS チーム名,
    consecutive_agari_count AS 連続あがり回数,
