...
WITH team_games AS (
    SELECT 
        ...
        ROW_NUMBER() OVER (PARTITION BY t.id, ls.start_year, ss.stage ORDER BY g.date, g.day_game_number) AS game_sequence
    ...
    GROUP BY ls.start_year, ss.stage, t.id, t.name, g.id, g.date, g.day_game_number
),
-- 連続記録をグループ化するための準備
streak_groups AS (
    SELECT 
