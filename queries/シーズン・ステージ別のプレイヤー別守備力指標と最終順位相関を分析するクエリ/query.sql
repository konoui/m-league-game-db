...
WITH player_defensive_stats AS (
  SELECT
    ...
    AVG(CASE WHEN ae.target_player_id = p.id THEN ae.revenue_points END) as avg_houjuu_points
  FROM player p
  -- プレイヤー・試合・シーズン情報の結合
  ...
  -- 局・イベント情報の結合（ron/tsumo のみに絞り idx_event_kyoku_type を活用）
  JOIN kyoku k ON g.id = k.game_id
  LEFT JOIN event e ON k.id = e.kyoku_id AND e.type IN ('ron', 'tsumo')
...
JOIN player_rankings pr ON pds.player_id = pr.player_id
