...
WITH player_games AS (
    SELECT 
        ...
        g.date,
        gpr.rank,
        CASE WHEN gpr.rank = 4 THEN 1 ELSE 0 END AS is_fourth,
...
streak_groups AS (
    SELECT 
        ...
        is_fourth,
        game_sequence,
        game_sequence - ROW_NUMBER() OVER (PARTITION BY player_name, season_year, stage, is_fourth ORDER BY game_sequence) AS streak_group
    FROM player_games
),
-- 各連続4着記録の長さを算出
...
-- 最終結果: 連続4着回数の上位10件を出力
SELECT 
    season_year AS シーズン年,
    stage AS ステージ,
    player_name AS プレイヤー名,
    team_name AS チーム名,
    streak_length AS 連続4着回数,
